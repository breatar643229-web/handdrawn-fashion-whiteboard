...
def _colored_paint_mask(color_img: np.ndarray, canvas_bgr: np.ndarray) -> np.ndarray:
    """定位蓝色衣服区域；头发、皮肤和灰色线稿不参与上色。"""
    hsv = cv2.cvtColor(color_img, cv2.COLOR_BGR2HSV)
    h = hsv[:, :, 0]
    s = hsv[:, :, 1]
    bgr = color_img.astype(np.int16)
    b, g, r = bgr[:, :, 0], bgr[:, :, 1], bgr[:, :, 2]
    return (h >= 85) & (h <= 125) & (s > 45) & (b > r + 25) & (b > g + 5)
...
class StreamBoardRenderer:
    ...
    def wash_color_contour(self, writer: cv2.VideoWriter, target_frames: int) -> None:
        ...
        paint_mask = _colored_paint_mask(self.color_img, self.canvas_bgr)
        paint_columns = np.where(paint_mask.any(axis=0))[0]
        ...
        sweep = h + 2 * delay_px
        report_step = max(1, target_frames // 10)
        smooth_cursor: tuple[float, float] | None = None
        for fi in range(target_frames):
            ...
            # 颜色前沿提前约 1 秒，避免出现“手扫过去后颜色才跟上”的滞后感。
            threshold = lead + wave[None, :] - resistance * delay_px  # (H,W)
            one_second_lead_px = int(round(sweep / max(1, target_frames - 1) * cfg.fps))
            color_sync_lead_px = max(36, int(round(h * 0.08)) + one_second_lead_px)
            reveal = ys <= (threshold + color_sync_lead_px)  # (H,W) bool

            # 只揭示真正有颜色的衣服区域；头发、脸和普通线稿不参与上色。
            active_paint = reveal & paint_mask
            self.drawn[active_paint] = color_src[active_paint]
            color_done = not np.any(paint_mask & ~reveal)
            if color_done:
                self.drawn[paint_mask] = color_src[paint_mask]
            ...
            raw_cursor_x = int(lane * w) if forward else int((1.0 - lane) * w)
            raw_cursor_x = max(0, min(w - 1, raw_cursor_x))

            # 光标贴在已经显色的衣服边缘上；这样手不会领先颜色太多。
            active_columns = np.where(active_paint.any(axis=0))[0]
            if active_columns.size > 0:
                target_x = int(active_columns[np.argmin(np.abs(active_columns - raw_cursor_x))])
                target_rows = np.where(active_paint[:, target_x])[0]
                target_y = int(target_rows[-1])
            elif paint_columns.size > 0:
                target_x = int(paint_columns[np.argmin(np.abs(paint_columns - raw_cursor_x))])
                target_rows = np.where(paint_mask[:, target_x])[0]
                target_y = int(target_rows[0])
            else:
                col_revealed = np.where(reveal[:, raw_cursor_x])[0]
                target_x = raw_cursor_x
                target_y = int(col_revealed[-1]) if col_revealed.size > 0 else 0

            if smooth_cursor is None:
                smooth_cursor = (float(target_x), float(target_y))
            else:
                sx, sy = smooth_cursor
                smooth_cursor = (sx * 0.30 + target_x * 0.70, sy * 0.30 + target_y * 0.70)
            cursor_x = int(round(smooth_cursor[0]))
            cursor_y = int(round(smooth_cursor[1]))

            # 最后几帧先收掉手，再进入干净成图展示，避免结尾突然闪一下。
            clean_tail_frames = max(1, int(round(cfg.fps * 0.15)))
            if color_done or fi >= target_frames - clean_tail_frames:
                writer.write(np.clip(self.drawn, 0, 255).astype(np.uint8))
            else:
        ...
        # 收尾兜底：只确保衣服彩色区域已揭示，保留头发等非彩色线稿状态。
        self.drawn[paint_mask] = color_src[paint_mask]
        last = np.clip(self.drawn, 0, 255).astype(np.uint8)
        while written < target_frames:
            writer.write(last)
            written += 1
    ...
    def render_to(self, raw_path: Path, total_ms: int) -> Path:
        ...
        # 凝视：完整原图
        gaze_img = np.clip(self.drawn, 0, 255).astype(np.uint8)
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--gaze-seconds", type=float, default=None, help="覆盖最后凝视展示秒数")
...
def _build_cfg(args: argparse.Namespace) -> Config:
    ...
    if args.gaze_seconds is not None:
        kw["gaze_seconds"] = max(0.0, args.gaze_seconds)
